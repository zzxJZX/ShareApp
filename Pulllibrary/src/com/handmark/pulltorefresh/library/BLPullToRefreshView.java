...
public class BLPullToRefreshView extends LinearLayout {
	...
	/** header view. */
	private BLListViewHeader mHeaderView;

	/** footer view. */
	private BLListViewFooter mFooterView;
	 ...
	 */
	public BLPullToRefreshView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	...
	private void addHeaderView() {
        mHeaderView = new BLListViewHeader(mContext);
	...
	private void addFooterView() {
        ...
        mFooterView = new BLListViewFooter(mContext);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_CANCEL:
			...
			if (mPullState == PULL_DOWN_STATE) {
				...
				System.out.println("加载的高度："+mHeaderViewHeight+","+mFooterViewHeight);
	...
	private void headerPrepareToRefresh(int deltaY) {
		...
		if (newTopMargin >= 0 && mHeaderView.getState() != BLListViewHeader.STATE_REFRESHING) {
		    //提示松开刷新
		    mHeaderView.setState(BLListViewHeader.STATE_READY);

		} else if (newTopMargin < 0 && newTopMargin > -mHeaderViewHeight) {
		    //提示下拉刷新
		    mHeaderView.setState(BLListViewHeader.STATE_NORMAL);
		}
	}
	...
	private void footerPrepareToRefresh(int deltaY) {
		...
		if (Math.abs(newTopMargin) >= (mHeaderViewHeight + mFooterViewHeight) && mFooterView.getState()  != BLListViewFooter.STATE_LOADING) {
			mFooterView.setState(BLListViewFooter.STATE_READY);
		} else if (Math.abs(newTopMargin) < (mHeaderViewHeight + mFooterViewHeight)) {
		    mFooterView.setState(BLListViewFooter.STATE_LOADING);
		}
	}
	...
	public void headerRefreshing() {
		...
		mHeaderView.setState(BLListViewHeader.STATE_REFRESHING);
	...
	public void onHeaderRefreshFinish() {
		...
		if(mAdapterView!=null){
			...
			if(mCount > 0){
				...
				mFooterView.setState(BLListViewFooter.STATE_EMPTY);
			}
		}else{
			mFooterView.setState(BLListViewFooter.STATE_READY);
		}
	...
	public void onFooterLoadFinish() {
		...
		if(mAdapterView!=null){
			...
			if(countNew > mCount){
				...
				mFooterView.setState(BLListViewFooter.STATE_NO);
			}
		}else{
			mFooterView.setState(BLListViewFooter.STATE_READY);
		}
